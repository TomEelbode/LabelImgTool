...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, filename=None):
        ...
        openNextImg = action('&Next Image', self.openNextImg, 'Right', 'next',
                             u'Open Next')

        openPrevImg = action('&Prev Image', self.openPrevImg, 'Left', 'prev',
                             u'Open Prev')
        ...
        self.actions = struct(
            ...
            onLoadActive=(close, ),
            onDETActive=(createRect, createMode, editMode),
            onSEGActive=(createPolygon, createMode, editMode),
            onCLSActive=(),
        ...
        addActions(self.menus.file,
                   (open, opendir, changeSavedir, openAnnotation,
                    self.menus.recentFiles, save, saveAs, remote_settings,
                    settings, login, close, None, quit))
        ...
        self.actions.beginner = (open, opendir, loadVideo, openNextImg,
                                 openPrevImg, save, None, createRect,
                                 createPolygon, copy, delete, None, zoomIn,
                                 zoom, zoomOut, fitWindow, fitWidth)

        self.actions.advanced = (open, save, loadVideo, None, createMode,
        ...
        if self.has_defined_color_map and len(self.label_color_map) < len(
                self.labelHist):
            print(
    ...
    def loadOnlineImages(self):
        if self.image_list:
            t = loadImageThread(self.database_url, self.image_list,
                                self.mImgList, self.loadFilePath)
            loadOnlineImgMul(self.database_url, self.image_list, 2,
                             self.mImgList, self.loadFilePath)
            while True:
                if self.mImgList:
    ...
    def saveLabels(self, filename):
        ...
        if self.task_mode == 1:  #seg mode
            ...
            mask_writer = label_mask_writer(self.label_num_dic, result_path,
                                            self.image_size[0],
                                            self.image_size[1])
        ...
        if self.task_mode in [0, 1]:  # seg and det mode
            ...
            savefilename = os.path.join(
                self.defaultSaveDir + os.path.splitext(imgFileName)[0] +
                '.png')  # the mask image will be save as file_mask.jpg etc.
    ...
    def newShape(self):
        ...
        if text is not None:
            ...
            if self.enable_color_map:
                fill_color = self.label_color_map[self.label_num_dic[text]]
                new_shape.fill_color = QColor(fill_color[0], fill_color[1],
                                              fill_color[2], fill_color[3])
            if self.enable_instance_seg:
                yes, no = QMessageBox.Yes, QMessageBox.No
                msg = u'Is it a new instance with id ' + str(
                    self.current_instance_id + 1)
                if yes == QMessageBox.question(self, u'Attention', msg,
                                               yes | no):
    ...
    def showNextFrame(self):
        ...
        self.frame_grabber.skip_frames(self.framesToSkip)
        cvimage = self.frame_grabber.next()
    ...
    def loadFile(self, filename=None):
        ...
        if QFile.exists(filename):
            ...
            if self.task_mode in [0, 1]:
                ...
                if self.defaultSaveDir is not None:
                    maskPath = os.path.join(self.defaultSaveDir,
                                            basename + '.png')
    ...
    def load_label_color_map(self):
        ...
        if self.label_color_map_path is None:
            self.label_color_map_path = os.path.join('data',
                                                     'label_color_map.txt')
        if os.path.exists(self.label_color_map_path):
            ...
            self.label_color_map = [
                color + [50] for color in COLORMAP.values()
            ]
    ...
    def loadPredefinedCLSClasses(self):
        ...
        if self.labelHist:
            num = 0
            assert len(self.labelHist
                       ) <= 255, 'the num of labels should be less than 255 '
    ...
    def loadPredefinedDETClasses(self):
        ...
        predefined_subclasses_path = os.path.join('data',
                                                  'predefined_sub_classes.txt')
    ...
    def loadVideo(self):
        ...
        if filename is not None:
            ...
            self.defaultSaveDir = os.path.join(os.path.split(filename)[0],
                                               'Annotation/' +
                                               str(self.username))
